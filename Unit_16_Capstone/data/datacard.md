Dataset Card & Source Attribution

To ensure academic and professional integrity, this project utilizes a public supply chain dataset. Below is the metadata card documenting the source, size, and license of the raw operational data.
...
DataCo Global (Supply chain logistics provider)
...
Shashwat (Kaggle Dataset Curator)
...
~15 MB compressed / ~45 MB uncompressed

Scale

180,519 rows × 53 features
...
Semantic Context & Educational Value

The DataCo dataset represents real-world, unnormalized transactional events from global delivery routes. It captures spatial data (origin and destination cities, states, and countries), financial metrics (product costs, order totals, and benefit margins), temporal dates (order placement vs. actual shipping timestamps), and operational risk factors (late delivery indicators). This scale makes it a perfect sandbox for students to experience structural normalization challenges, index optimization, and downstream machine learning integrations inside an enterprise-grade environment.
