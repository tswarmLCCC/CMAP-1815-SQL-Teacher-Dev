...
* **Period 1 (In-Class): Environment Setup & Database Basics** *(Difficulty: 2/5 - Technical setup can have hiccups)*
  ...
  * **Resource Guidance:** [Watch this part of the video from 0:00:00 to 0:35:00](https://www.youtube.com/watch?v=qw--VYLpxG4&t=0s) to understand database architecture and the installation process.
...
* **Period 2 (In-Class): The `SELECT` Statement & Basic Formatting** *(Difficulty: 1/5 - Very straightforward syntax)*
  ...
  * **Resource Guidance:** [Click here and read the "PostgreSQL SELECT" section](https://neon.com/postgresql/tutorial) from the sidebar to review basic query syntax.
...
* **Periods 3 & 4 (Independent): Data Exploration Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **Environment & Navigation (45 mins):** *(Difficulty: 2/5)* Configure the PostgreSQL connection. Identify the target schema and document primary keys without writing code. **Resource:** [Watch the DBeaver/GUI setup part of the video from 0:35:00 to 0:50:00](https://www.youtube.com/watch?v=qw--VYLpxG4&t=2100s).

    * **Guided Queries (45 mins):** *(Difficulty: 1/5)* Write 15-20 foundational queries (e.g., retrieving full names, math operations). **Resource:** [Read this section on "Column Aliases"](https://neon.com/postgresql/tutorial) for formatting reference.

    * **Exploration Challenge (60 mins):** *(Difficulty: 2/5)* Given business questions, determine which tables contain the data and write `SELECT` statements to extract it.
...
* **Period 1 (In-Class): The `WHERE` Clause** *(Difficulty: 2/5)*
  ...
  * **Resource Guidance:** [Click here and read from "PostgreSQL WHERE" to "PostgreSQL LIKE"](https://neon.com/postgresql/tutorial) in the sidebar.
...
* **Period 2 (In-Class): Advanced Filtering & Sorting** *(Difficulty: 3/5 - Boolean logic is tricky for some)*
  ...
  * **Resource Guidance:** [Read the "PostgreSQL AND/OR" and "PostgreSQL ORDER BY" sections here](https://neon.com/postgresql/tutorial).
...
* **Periods 3 & 4 (Independent): Targeted Retrieval Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **Logic Drills (45 mins):** *(Difficulty: 3/5)* Practice combining multiple conditions and identifying logic errors in failing queries.

    * **Pattern Matching Scenarios (45 mins):** *(Difficulty: 2/5)* Use `LIKE` and wildcards to find poorly formatted data.

    * **Business Reporting Challenge (60 mins):** *(Difficulty: 3/5)* Produce specific, ordered lists requiring combination of logic (e.g., top 10 most expensive items currently in stock). **Resource:** [Click here and keep the "Filtering Data" modules open](https://neon.com/postgresql/tutorial) for syntax reference during the challenge.
...
* **Period 1 (In-Class): Relational Concepts & Inner Joins** *(Difficulty: 3/5 - First major conceptual hurdle)*
  ...
  * **Resource Guidance:** [Watch this part of the video from approximately 1:30:00 to 1:55:00](https://www.youtube.com/watch?v=qw--VYLpxG4&t=5400s) to visualize Venn diagrams and row-matching for Inner Joins.
...
* **Period 2 (In-Class): Outer Joins & Complex Joins** *(Difficulty: 4/5)*
  ...
  * **Resource Guidance:** [Watch this part of the video from 1:55:00 to 2:20:00](https://www.youtube.com/watch?v=qw--VYLpxG4&t=6900s) to understand Outer and Complex table joins.
...
* **Periods 3 & 4 (Independent): Data Integration Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **ERD Navigation (30 mins):** *(Difficulty: 2/5)* Trace relationships on a provided Entity Relationship Diagram to map paths between distant tables.

    * **Join Mechanics (60 mins):** *(Difficulty: 4/5)* Write 4-table joins. **Resource:** [Read this "PostgreSQL Joins" section](https://neon.com/postgresql/tutorial) to look up the exact ON/USING clause syntax while coding.

    * **The "Missing Data" Audit (60 mins):** *(Difficulty: 4/5)* Utilize `LEFT JOIN` and `IS NULL` to identify anomalies (e.g., finding products that have never been ordered).
...
* **Period 1 (In-Class): Aggregation & Grouping** *(Difficulty: 3/5)*
  ...
  * **Resource Guidance:** [Click here and read the "PostgreSQL GROUP BY" and "PostgreSQL HAVING" sections](https://neon.com/postgresql/tutorial).
...
* **Period 2 (In-Class): Set Operations & Basic Pivoting** *(Difficulty: 5/5 - Conditional aggregation is highly advanced for intro level)*
  ...
  * **Resource Guidance:** [Read this "PostgreSQL Set Operations" section](https://neon.com/postgresql/tutorial) to understand UNION rules.
...
* **Periods 3 & 4 (Independent): Business Reporting Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **Summary Reports (45 mins):** *(Difficulty: 3/5)* Use `GROUP BY` and `HAVING`. **Resource:** [Click here and reference "PostgreSQL Aggregate Functions"](https://neon.com/postgresql/tutorial) during this section.

    * **Set Operations (45 mins):** *(Difficulty: 2/5)* Use `UNION` to combine distinct datasets.

    * **The Pivot Challenge (60 mins):** *(Difficulty: 5/5)* Take row-based data and use conditional aggregation to pivot it into column-based data. *(Prime candidate for pruning if pacing is behind)*.
...
* **Period 1 (In-Class): DML & The Art of Validation** *(Difficulty: 3/5)*
  ...
  * **Resource Guidance:** [Read from the "PostgreSQL INSERT" through "PostgreSQL DELETE" sections here](https://neon.com/postgresql/tutorial) to ensure strict adherence to syntax.
...
* **Period 2 (In-Class): Temporary Tables** *(Difficulty: 3/5)*
  ...
  * Creating and using Local Temp Tables (`#Table` or `TEMP TABLE`).
...
* **Periods 3 & 4 (Independent): Data Cleanup Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **The Safe Update Protocol (60 mins):** *(Difficulty: 4/5)* Submit the `SELECT` query used to identify target rows, the `UPDATE` statement, and the post-execution verification query. **Resource:** [Click here and keep the "Modifying Data" section open](https://neon.com/postgresql/tutorial) to prevent catastrophic UPDATE errors.

    * **Data Archiving (60 mins):** *(Difficulty: 4/5)* Use a multi-step process involving Temp Tables to archive and delete old records, documenting row counts.
...
* **Period 1 (In-Class): Subqueries & Common Table Expressions (CTEs)** *(Difficulty: 4/5)*
  ...
  * **Resource Guidance:** [Read the "PostgreSQL CTE" section here](https://neon.com/postgresql/tutorial).
...
* **Period 2 (In-Class): Window Functions** *(Difficulty: 5/5 - Heavy conceptual abstraction)*
  ...
  * **Resource Guidance:** [Read the "PostgreSQL Window Functions" section here](https://neon.com/postgresql/tutorial).
...
* **Periods 3 & 4 (Independent): Advanced Analytics Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **Refactoring Exercise (45 mins):** *(Difficulty: 4/5)* Refactor a highly nested query into a clean chain of CTEs.

    * **Ranking & Deduplication (45 mins):** *(Difficulty: 4/5)* Use `ROW_NUMBER()` and `PARTITION BY` to rank items and filter out duplicates. **Resource:** [Read these window function examples](https://neon.com/postgresql/tutorial) to complete the ranking exercises.

    * **Running Totals & Moving Averages (60 mins):** *(Difficulty: 5/5)* Use Window Functions to calculate running totals and 7-day moving averages. *(Can be pruned if class is struggling)*.
...
* **Period 1 (In-Class): Design & Integrity** *(Difficulty: 4/5 - Normalization is highly theoretical)*
  ...
  * **Resource Guidance:** [Watch this part of the video from approximately 3:00:00 to 3:45:00](https://www.youtube.com/watch?v=qw--VYLpxG4&t=10800s) covering Database Design, Primary/Foreign Keys, and Constraints.
...
* **Period 2 (In-Class): Custom Database Objects & Intro to Data Warehousing** *(Difficulty: 3/5)*
  ...
  * **Resource Guidance:** [Read the "PostgreSQL Views" section here](https://neon.com/postgresql/tutorial).
...
* **Periods 3 & 4 (Independent): Schema Design Lab**
  ...
  * *150-Minute Activity Breakdown:*
    ...
    * **DDL Scripting (60 mins):** *(Difficulty: 3/5 - Easy concepts, syntax-heavy)* Write the exact `CREATE TABLE` scripts with correct constraints. **Resource:** [Click here and read the "PostgreSQL Data Types" and "CREATE TABLE" sections](https://neon.com/postgresql/tutorial).

    * **Enforcing the Rules (30 mins):** *(Difficulty: 3/5)* Write `ALTER TABLE` statements to add keys and constraints.

    * **Custom Objects (15 mins):** *(Difficulty: 2/5)* Create a simple View that encapsulates a complex, multi-table query.
...
* **Period 1 (In-Class): Performance & Indexing Basics** *(Difficulty: 3/5)*
  ...
  * **Resource Guidance:** [Read the "PostgreSQL Indexes" section here](https://neon.com/postgresql/tutorial).
